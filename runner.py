...
logging.basicConfig(level=logging.INFO, filename='/tmp/x-runner.log', format='%(asctime)s - %(filename)s - %(lineno)s - %(message)s')
...
class Runner(dbus.service.Object):
    ...
    def Match(self, query: str):
        ...
        results = []
        if query.startswith('wez') or 'theme' in query:
            log.info('wez theme updating request')
            results = results + [
                ('wez-theme', 'change theme', "user-busy-symbolic", 100, 1, {'subtext': 'Update Wez Theme', 'actions': ['start']})
            ]

        results = results + [
            # (hostname, f'{hostname} - {datetime.now().strftime("%H:%M:%S")}', "user-busy-symbolic", 100, rel/10, {'subtext': 'SubText'})
            # for rel, hostname in enumerate(['a34', 'a46'])
        ]
    ...
    def Actions(self):
        ...
        Structure is:
          ...
          - IconName
        """
        log.info(f"{'+' * 10} actions, Received an Actions query, called upon initialization")
        # ID, Text, IconName
        return [
    ...
    def Run(self, match_id: str, action_id: str, *args, **kwargs):
        ...
        log.info("run, received a Run command, match_id: %s, action_id: %s, %s, %s", match_id, action_id, args, kwargs)
